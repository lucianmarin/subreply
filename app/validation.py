...
def valid_content(value, username, user_id):
    ...
    if not value:
        ...
        if duplicate:
            msg = 'Same status of <a href="/{0}">@{0}</a>'
            return msg.format(duplicate.created_by)
        # elif all(uid == user_id for uid in last_uids):
        #     return "Please wait your turn"


def discussion(content, user):
    errors = {}
    errors['content'] = [valid_content(content, user.username, user.id)]
    errors = {k: v for k, v in errors.items() if v[0]}
    return errors
